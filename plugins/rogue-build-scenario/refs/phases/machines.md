...
**LAW 1.5: Addon Config Samples take precedence over hand-rolled config.** The `list_full` response carries `addonConfigSamples[{ sampleId, name, notes, language }]` per plugin version. When `addonConfigSampleCount > 0`, scan the `notes` fields to find a sample matching the machine's character backstory / role / story (notes are the discovery surface — names are typically terse). Fetch the chosen sample's code with `architect_plugin_catalog_get_addon_config_sample({ sampleId })`, tweak in-memory (rename users to the scenario's `samAccountName`s, swap hostnames, adjust file paths), and feed it into the plugin's `stringBlock` param via `_set_params`. Writing a fresh timeline / rule set / profile when a working sample already ships is a silent regression — defaults are sample-first. Distinct from `architect_plugin_catalog_get_example` — that tool returns plugin USAGE examples (how a plugin was wired into a real scenario); this one returns CONFIG content the plugin operates on at runtime.
...
6. **Discover parameter field names + Addon Config Samples** — Call `architect_plugin_catalog_list_full` with all `pluginVersionId`s from the machine's plugin list to get exact parameter names, types, descriptions, CSV headers, AND the `addonConfigSamples[]` summary list per plugin version. Always precedes step 7. For every plugin whose returned `addonConfigSampleCount > 0`, hold the summary list in working memory — step 7 decides whether to pull a sample.
7. **Configure parameters** — Call `architect_assigned_plugin_set_params` using the exact field names from step 6 and the parameter descriptions to choose values. **For plugins that ship Addon Config Samples** (`addonConfigSampleCount > 0` in step 6): before authoring any large `stringBlock` value from scratch, scan the cached `addonConfigSamples[].notes` for a match against the scenario's intent for this machine (assigned user's role, machine purpose narrative, exploit hop's needs). On match, call `architect_plugin_catalog_get_addon_config_sample({ sampleId })` to fetch the code, tweak in memory (substitute SAMs, hostnames, IPs, file paths), then pass the tweaked blob into `_set_params`. On failure, check the error for correct field names and retry. For DC CSV params (CreateUsers, CreateOUs, CreateGroups), follow the Staged Dispatch Pattern per "DC User CSV Generation" below — the orchestrator authors identity fields, subagents author narrative fields, the orchestrator assembles and pushes via staging. Samples are for `stringBlock` runtime config — DC CSVs are scenario-identity, not runtime config.
