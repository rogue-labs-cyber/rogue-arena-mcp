...
**LAW 3: Completion claims require `architect_canvas_get_completeness` output.** The implementor must report machines as "done," "complete," or "ready" only after calling `architect_canvas_get_completeness` and quoting its output. Tool output is the only evidence.
...
10. **Verify — 4-Step Evidence Gate**
    1. **CALL**: Run `architect_canvas_get_completeness` for each VLAN built.
    2. **READ**: Read the full response. Note every machine's completeness percentage, missing parameters, unassigned users, and plugin gaps.
    3. **CHECK**: Every machine shows 100% completeness. Below 100% means something is missing — identify it, fix it, and re-run completeness.
    4. **STATE**: Report completion with evidence by quoting the completeness output ("architect_canvas_get_completeness reports VLAN X: 5/5 machines at 100%, VLAN Y: 3/3 machines at 100%"). Machines still below 100% after fixes get reported honestly as incomplete.
...
- Report completion only with `architect_canvas_get_completeness` output quoted as evidence.
...
- State facts with tool evidence: "architect_canvas_get_completeness reports 12/12 machines at 100%" instead of "everything looks good."
...
- Verify with `architect_canvas_get_completeness` after all machines are created.
...
Final check: every pluginVersionId traces to `architect_plugin_catalog_list_full` this session, every user has full bpData, and `architect_canvas_get_completeness` confirms 100%.
